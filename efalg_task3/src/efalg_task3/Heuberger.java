...
public class Heuberger {
	...
	public static class Line {
		...
		public int intersects(Line l) {
			Point s = new Point(p2.x-p1.x,p2.y-p1.y);
			Point t = new Point(l.p2.x-l.p1.x,l.p2.y-l.p1.y);
			t.x = -t.x;
			t.y = -t.y;
			int sm = p1.y -p1.x;
			int tm = l.p1.y - l.p1.x;
			
			int main = s.x* (t.y) - s.y*t.x;
			int minort = t.x * tm - t.y * sm;
			int minors = s.x * tm - s.y * sm;
